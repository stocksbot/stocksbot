...
class Boards(commands.Cog):
    ...
    async def globalboard(self, ctx: commands.Context, size=10):
        ...
        for player in ordered_users:
            ...
            temp.append(player.name)
            temp.append(str(player.tag))
            temp.append(str(player.guild_name))
            ...
            temp.append(str(net_worth))
            global_leaderboard.append(temp)

        # sort leaderboard by decreasing net worth
        sorted_global_board = sorted(global_leaderboard, key=lambda x:float(x[3]), reverse=True)

        # Append empty local board contents (for aesthetics in case there are less players than the size of board)
        if len(sorted_global_board) < size:
            for i in range(size - len(sorted_global_board)):
                sorted_global_board.append(["----","----","----","----"])
    
        # output according to the desired size
        output = sorted_global_board[:size]
